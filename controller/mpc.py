...
class ARXMpc():
    ...
    def _get_forward_func(self):
        def forward(x_slided, u_slided,p=None,tvp=None):
            ...
            res = jnp.matmul(input_model , self.model.core.coef_ )
            return res
    ...
    def _treat_y(self, y):
        ...
        # target order => ["u", "weaSta_reaWeaTDryBul_y", "weaSta_reaWeaRelHum_y","time", "weaSta_reaWeaHGloHor_y", "weaSta_reaWeaSolAlt_y"]
    ...
    def _preprocessing_measurements(self, measurement):
        return None
    ...
    def update(self, measurements, forcasts):
        ...
        proxy_model.set_prev_data(x_prev=self.prev_y[:,0:1], u_prev=self.prev_u, tvp_prev=self.prev_y[:,1:]) # TODO

        constraints_nmpc = [nEMPC.constraints.DomainConstraint(
                    states_constraint=[[-np.inf, np.inf],],
                    control_constraint=[[0.0,1.0],]),]

        objective_func  =  nEMPC.objective.jax.JAXObjectifFunc(self._get_cost_func())
